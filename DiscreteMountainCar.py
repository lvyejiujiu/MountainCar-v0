...
def getAction(position, velocity, valueFunction,env):
...
# @n: # of steps
def semiGradientNStepSarsa(valueFunction, env, n=1):
    # start at a random position around the bottom of the valley

    [currentPosition, currentVelocity] = env.reset()
    currentAction = getAction(currentPosition, currentVelocity, valueFunction, env)
    ...
    while True:
        ...
        if time < T:
            ...
            newVelocity = state[1]
            # choose new action
            newAction = getAction(newPostion, newVelocity, valueFunction, env)
...
def policyReplay(valueFunction,env):
    [currentPosition, currentVelocity] = env.reset ()
    currentAction = getAction (currentPosition, currentVelocity, valueFunction, env)
    # track previous position, velocity, action and reward
    Reward = 0.0
    ...
    while True:
        ...
        currentAction = newAction
        if time == T:
            print ('In policy replay, finish one episode')
            break
    return Reward, T
...
def runTest():
    ...
    episodes = 500
    StopTime = 140
    PolicyRePlayEpisode = 10
    #targetEpisodes = [0, 99, episodes - 1]
    numOfTilings = 8
    alpha = 0.3
    valueFunction = ValueFunction(alpha, numOfTilings)
    TimeList = deque(maxlen = 10)
    for episode in range(0, episodes):
        print('episode:', episode)
        time = semiGradientNStepSarsa(valueFunction,env)
        print ('the lasting time step is: ', time)
        TimeList.append(time)
        if np.mean(TimeList) < StopTime:
            break
    print ('----------------------------------')
    for Replay in range (PolicyRePlayEpisode):
        print ('Policy replay No: ', Replay)
        PolicyReward, PolicyTimes= policyReplay(valueFunction , env)
        print ('PolicyReward = ', PolicyReward)
        print ('PolicyTimes = ', PolicyTimes)
